...
- `benchmarks/data/eval_vi_20260908.jsonl` — **12 Vietnamese news articles** (250–900 words)
  with human reference summaries, drawn deterministically (seed 42). Each entry carries a
  `clean` flag; entries flagged `false` overlap a corpus that one of the compared checkpoints
  was trained on (a memorization guard). The official table below is computed on the **clean
  subset**; flagged entries stay in the data file so the exclusion is auditable rather than
  silent.
...
## Official results — clean subset (full per-doc: `results/2026-09-08/`)
...
*Remaining archived checkpoints score ROUGE-2 0.08–0.16 — see
`results/2026-09-08/per_doc.csv`.*
