...
DUPLICATES = ("vit5_base_vietnews_summarization_copy", "vit5_v1_fp16")  # same weights
...
def load_tok(bd):
    """Try tokenizer strategies in order; some artifacts predate transformers 5.x."""
    attempts = (
        lambda: AutoTokenizer.from_pretrained(str(bd)),
        lambda: T5TokenizerFast(tokenizer_file=str(bd / "tokenizer.json")),
        lambda: AutoTokenizer.from_pretrained(str(bd), use_fast=False),
    )
    last_err = None
    for attempt in attempts:
        try:
            ...
            last_err = e
    raise RuntimeError(f"no tokenizer: {last_err}") from last_err
...
def main():
    ...
    ap.add_argument("--include-dups", action="store_true",
                    help="also benchmark vit5_base_vietnews_summarization_copy + vit5_v1_fp16 "
                         "(same weights as other entries; skipped by default)")
    args = ap.parse_args()
    ...
    if not args.include_dups:
        dirs = [d for d in dirs if d.name not in DUPLICATES]
    ...
    with open(outdir / "per_doc.csv", "w", newline="", encoding="utf-8") as csv_fh:
        csv_w = csv.DictWriter(csv_fh, fieldnames=["model", "doc", "clean", "r2", "copy5",
                                                   "num_hall", "tri", "gen_s", "hyp"])
        csv_w.writeheader()
        for d in dirs:
            ...
            for doc in pool:
                ...
                ht, rt, st = toks(hyp), toks(doc["summary"]), toks(doc["text"])
                vals = {
                    "r1": rouge1(ht, rt), "r2": rouge2(ht, rt), "rl": rouge_l(ht, rt),
                    "c5": copy5(ht, st), "nh": num_halluc(hyp, doc["text"]),
                    "tr": tri_repeat(ht), "gt": round(time.time() - tin, 1),
                }
                for k, agg_list in m_all.items():
                    agg_list.append(vals[k])
                    if clean:
                        m_clean[k].append(vals[k])
                csv_w.writerow({
                    "model": d.name, "doc": doc["id"], "clean": clean, "r2": vals["r2"],
            ...
            def agg_of(m):
                return {
                    "r1": round(mean(m["r1"]), 4), "r1_sd": round(stdev(m["r1"]), 4),
    ...
    from scipy import stats  # heavy import only when stats are needed
